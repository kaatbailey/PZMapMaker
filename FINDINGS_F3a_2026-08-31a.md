...
**Status:** complete. Divergence characterised; **owner decided option 2 —
accept and document** (2026-08-31).
...
`minAreaRect` divergences, on the same lines — across GCC 13 and GCC 16, both
x86-64 / OpenJDK 21. A stable reproducible difference, not chaotic noise. This
is what made option 2 defensible.

**OWNER DECISION 2026-08-31: OPTION 2 — ACCEPT AND DOCUMENT.** Not fixed.
**Trigger for revisiting:** F6 shows an unexplained diff on real data. That is
the moment to apply the fix to both trees, not before.

Applying this to C++ alone makes the two trees
...
**Unverified:**
- Whether the `atan2` divergence differs across JDK vendors or CPU
  architectures. Two GCC majors on x86-64 / OpenJDK 21 agree exactly; ARM or a
  non-HotSpot JVM is untested. If it varies there, option 1 becomes the only
  stable choice.
