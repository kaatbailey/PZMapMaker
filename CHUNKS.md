...
| `[~]` | **C4** Tool layer: brushes, selection, undo UI | C3, A3 | **Picking + floor replacement done 2026-08-25.** Click → tile names/properties, hover+selection overlays, and Set Floor writes one square via CellEditor::setFloor (edit→dirty→save→reload verified on 41_37). Working level persists; edit path preserves camera. Learned: floor sprites aren't all 1×1 (some are 2×2-block art) — palette thumbnails must show real extent. Next: tool palette + palette-preload atlas, then walls/objects/undo UI. See STATE §"C4 write side". |
