> **THIS COPY IS STALE AND FROZEN. The canonical `STATE.md` lives in
> `~/Documents/PZMapMaker`.**
>
> Resolved 2026-09-01 (STATE §40). §37 recorded these two copies as having
> "diverged" and set the test as `diff` them. Done: **`diff` returns ZERO
> lines unique to this copy.** It is a strict subset — a snapshot frozen at
> §35, 3,344 lines against the canonical 4,867. §37's supporting figure was
> also wrong: water mentions are 38 vs 2, not 7 vs 8.
>
> **Nothing was lost and no merge is required.** Read the PZMapMaker copy.
> This file is kept unaltered below because CHARTER §5 forbids deleting from
> `STATE.md`, and because per CHARTER §2 this repo is archive-only.
>
> Everything after §35 — the entire C++ port track, §36 through §40 — exists
> only in the canonical copy.
